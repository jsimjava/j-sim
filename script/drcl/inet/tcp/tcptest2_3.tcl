# tcptest2_3.tcl
#
# same as tcptest2_2.tcl but use different MSS for different TCP sessions
#
# Topology:
# h0 ---\               /--- h5
#        \             /
...
! h0,h5/tcp* setMSS 512; # bytes
! h0,h5/s* setDataUnit 512
! h1,h6/tcp* setMSS 1024; # bytes
! h1,h6/s* setDataUnit 1024
! h2,h7/tcp* setMSS 1536; # bytes
! h2,h7/s* setDataUnit 1536

# Configure the bottleneck bandwidth and buffer size
! n3 setBandwidth 3 1.0e6; # 1Mbps at interface 3
! n4 setBandwidth 0 1.0e6; # 1Mbps at interface 0
#! n3 setBuffer 3 9 packet; # 9+1 packets at interface 3
! n3 setBuffer 3 20000 byte; # 9+1 packets at interface 3
...
foreach i {5 6 7} {
	set j [expr $i-5]
	if 1 {;# throughput
		set tm_ [mkdir drcl.comp.tool.CountMonitor .tm$i]
		$tm_ configure 100.0 4.0; # window size, update interval
		connect -c h$i/csl/6@up -to .tm$i/in@
		connect -c .tm$i/sizecount@ -to $plot_/$j@0
	}
