...
proc grep {pattern_ content_} {
	...
	set p [java::call java.util.regex.Pattern compile $pattern_]

	foreach ll $lines_ {
		#if [string match "*$pattern_\*" $ll] { append result_ "$ll\n" }
		#if [regexp -- $pattern_ $ll] { append result_ "$ll\n" }
		set m [$p matcher [java::new String $ll]]
		if [$m find] { append result_ "$ll\n" }
	}
	return $result_
}
